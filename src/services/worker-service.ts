...
import { RAGRoutes } from './worker/http/routes/RAGRoutes.js';
...
export class WorkerService {
  ...
  private registerRoutes(): void {
    ...
    this.server.registerRoutes(new RAGRoutes(this.dbManager));
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      // Start periodic cleanup job (every 10 minutes)
      this.startPeriodicCleanup(pendingStore);
    } catch (error) {
      logger.error('SYSTEM', 'Background initialization failed', {}, error as Error);
      throw error;
    }
  ...
  /**
   * Start periodic cleanup job for stuck messages and stale sessions
   * Runs every 10 minutes to prevent accumulation of orphaned resources
   */
  private startPeriodicCleanup(pendingStore: InstanceType<typeof import('./sqlite/PendingMessageStore.js').PendingMessageStore>): void {
    const CLEANUP_INTERVAL_MS = 10 * 60 * 1000; // 10 minutes
    const STUCK_THRESHOLD_MS = 5 * 60 * 1000;   // 5 minutes
    const STALE_SESSION_HOURS = 4;               // 4 hours

    const cleanup = () => {
      try {
        // 1. Reset stuck messages (processing > 5 min)
        const stuckReset = pendingStore.resetStuckMessages(STUCK_THRESHOLD_MS);
        if (stuckReset > 0) {
          logger.info('CLEANUP', `Reset ${stuckReset} stuck messages`, { thresholdMinutes: 5 });
        }

        // 2. Mark stale sessions as completed
        const activeSessions = this.sessionManager.getActiveSessions();
        const excludeIds = activeSessions.map(s => s.sessionDbId);
        const store = this.dbManager.getSessionStore();
        const staleCleaned = store.markStaleSessions(STALE_SESSION_HOURS * 60 * 60 * 1000, excludeIds);
        if (staleCleaned > 0) {
          logger.info('CLEANUP', `Marked ${staleCleaned} stale sessions as completed`, { thresholdHours: STALE_SESSION_HOURS });
        }

        // 3. Clear old processed messages (> 24 hours)
        const oldProcessedCleared = pendingStore.clearOldProcessed(24 * 60 * 60 * 1000);
        if (oldProcessedCleared > 0) {
          logger.debug('CLEANUP', `Cleared ${oldProcessedCleared} old processed messages`, { thresholdHours: 24 });
        }
      } catch (error) {
        logger.error('CLEANUP', 'Periodic cleanup failed', {}, error as Error);
      }
    };
    ...
    logger.info('SYSTEM', 'Periodic cleanup job started', { intervalMinutes: 10 });
  }
