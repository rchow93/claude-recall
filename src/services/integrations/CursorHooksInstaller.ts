/**
 * CursorHooksInstaller - Cursor IDE integration for claude-recall
 *
...
export function findCursorHooksDir(): string | null {
  const possiblePaths = [
    ...
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'askqai', 'cursor-hooks'),
...
export function findMcpServerPath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'askqai', 'plugin', 'scripts', 'mcp-server.cjs'),
    // Development/source location (relative to built worker-service.cjs in plugin/scripts/)
...
export function findWorkerServicePath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'askqai', 'plugin', 'scripts', 'worker-service.cjs'),
    // Development/source location (relative to built worker-service.cjs in plugin/scripts/)
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/askqai/plugin/scripts/mcp-server.cjs');
    return 1;
  }
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  console.log(`\nInstalling Claude-Recall Cursor hooks (${target} level)...\n`);
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/askqai/plugin/scripts/worker-service.cjs');
    return 1;
  }
...
Next steps:
  1. Start claude-recall worker: claude-recall start
  2. Restart Cursor to load the hooks
  3. Check Cursor Settings → Hooks tab to verify

Context Injection:
  Context from past sessions is stored in .cursor/rules/claude-recall-context.mdc
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  if (!contextGenerated) {
    ...
    const rulesFile = path.join(rulesDir, 'claude-recall-context.mdc');
...
Use claude-recall's MCP search tools for manual memory queries.
`;
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  console.log(`\nUninstalling Claude-Recall Cursor hooks (${target} level)...\n`);
  ...
  try {
    ...
    if (target === 'project') {
      const contextFile = path.join(targetDir, 'rules', 'claude-recall-context.mdc');
...
export function checkCursorHooksStatus(): number {
  console.log('\nClaude-Recall Cursor Hooks Status\n');
  ...
  for (const loc of locations) {
    ...
    if (existsSync(hooksJson)) {
      ...
      if (loc.name === 'Project') {
        const contextFile = path.join(loc.dir, 'rules', 'claude-recall-context.mdc');
        if (existsSync(contextFile)) {
  ...
  if (!anyInstalled) {
    console.log('No hooks installed. Run: claude-recall cursor install\n');
  }
  ...
  switch (subcommand) {
    case 'install': {
      ...
      if (!cursorHooksDir) {
        ...
        console.error('   Expected at: ~/.claude/plugins/marketplaces/askqai/cursor-hooks/');
        return 1;
      }
    ...
    default: {
      console.log(`
Claude-Recall Cursor Integration

Usage: claude-recall cursor <command> [options]
...
Examples:
  ...
  claude-recall cursor install user         # Install globally for user
  claude-recall cursor uninstall            # Remove from current project
  claude-recall cursor status               # Check if hooks are installed

For more info: https://docs.claude-recall.ai/cursor
      `);
      return 0;
    }
  }
