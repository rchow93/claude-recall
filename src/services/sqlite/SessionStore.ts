...
export class SessionStore {
   ...
   */
  markSessionCompleted(contentSessionId: string): boolean {
    const now = new Date();
   ...
   */
  markSessionCompletedById(sessionDbId: number): boolean {
    const now = new Date();
    const nowEpoch = now.getTime();
   ...
   */
  markStaleSessions(maxAgeMs: number = 2 * 60 * 60 * 1000, excludeSessionDbIds: number[] = []): number {
    const now = new Date();
    const nowEpoch = now.getTime();
    const cutoffEpoch = nowEpoch - maxAgeMs;

    // Build exclusion clause
    const exclusionClause = excludeSessionDbIds.length > 0
      ? `AND id NOT IN (${excludeSessionDbIds.join(',')})`
      : '';
