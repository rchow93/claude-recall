 ...
 * - TieredSearchStrategy: pgvector hybrid search via QueryRouter (Redis + PostgreSQL)
...
export abstract class BaseSearchStrategy implements SearchStrategy {
   ...
   */
  protected emptyResult(strategy: 'tiered' | 'sqlite'): StrategySearchResult {
    return {
      results: {
        observations: [],
        sessions: [],
