/**
 * SearchManager - Core search orchestration for claude-recall
 *
...
export class SearchManager {
  ...
  constructor(
    ...
    // Initialize the modular search infrastructure
  ...
  /**
   * Query tiered search via QueryRouter
   */
  private async queryTiered(
    query: string,
    limit: number,
    project?: string
  ): Promise<{ obsIds: number[]; sessionIds: number[] }> {
    if (!this.queryRouter) {
      return { obsIds: [], sessionIds: [] };
    }
    ...
    return {
      obsIds: ragResult.observations.map(r => r.item.id),
      sessionIds: ragResult.summaries.map(r => r.item.id)
    };
  }
  ...
  async search(args: any): Promise<any> {
    ...
    // PATH 1: FILTER-ONLY (no query text) - use direct SQLite filtering
    if (!query) {
    ...
    // PATH 2: TIERED SEMANTIC SEARCH (query text + QueryRouter available)
    else if (this.queryRouter) {
      logger.debug('SEARCH', 'Using tiered pgvector semantic search', { typeFilter: type || 'all' });

      const tieredResults = await this.queryTiered(query, 100, options.project);

      if (tieredResults.obsIds.length > 0 && searchObservations) {
        const obsOptions = { ...options, type: obs_type, concepts, files };
        observations = this.sessionStore.getObservationsByIds(tieredResults.obsIds, obsOptions);
      }
      if (tieredResults.sessionIds.length > 0 && searchSessions) {
        sessions = this.sessionStore.getSessionSummariesByIds(tieredResults.sessionIds, { orderBy: 'date_desc', limit: options.limit, project: options.project });
      }

      logger.debug('SEARCH', 'Hydrated results from SQLite', { observations: observations.length, sessions: sessions.length });
    }
    // QueryRouter not initialized - mark as failed
    else if (query) {
      tieredFailed = true;
      logger.debug('SEARCH', 'Tiered search not initialized - semantic search unavailable', {});
      observations = [];
      sessions = [];
      prompts = [];
    }
  ...
  async timeline(args: any): Promise<any> {
    ...
    if (query) {
      ...
      if (this.queryRouter) {
        try {
          logger.debug('SEARCH', 'Using tiered semantic search for timeline query', {});
          const tieredResults = await this.queryTiered(query, 100);
          logger.debug('SEARCH', 'Tiered search returned semantic matches for timeline', { matchCount: tieredResults.obsIds.length });

          if (tieredResults.obsIds.length > 0) {
            results = this.sessionStore.getObservationsByIds(tieredResults.obsIds, { orderBy: 'date_desc', limit: 1 });
          }
        } catch (tieredError) {
          logger.error('SEARCH', 'Tiered search failed for timeline, continuing without semantic results', {}, tieredError as Error);
        }
      }
    ...
    if (this.queryRouter) {
      try {
        if (query) {
          ...
          logger.debug('SEARCH', 'Using tiered semantic search with type=decision filter', {});
          const tieredResults = await this.queryTiered(query, Math.min((filters.limit || 20) * 2, 100));
          const obsIds = tieredResults.obsIds;

          if (obsIds.length > 0) {
            results = this.sessionStore.getObservationsByIds(obsIds, { ...filters, type: 'decision' });
            // Preserve tiered ranking order
            results.sort((a, b) => obsIds.indexOf(a.id) - obsIds.indexOf(b.id));
          }
        } else {
          // No query: get all decisions, rank by "decision" keyword
          logger.debug('SEARCH', 'Using metadata-first + tiered semantic ranking for decisions', {});
          const metadataResults = this.sessionSearch.findByType('decision', filters);

          if (metadataResults.length > 0) {
            const ids = metadataResults.map(obs => obs.id);
            const tieredResults = await this.queryTiered('decision', Math.min(ids.length, 100));
            const tieredIdSet = new Set(tieredResults.obsIds);

            const rankedIds: number[] = [];
            for (const id of ids) {
              if (tieredIdSet.has(id)) rankedIds.push(id);
            }
            // Append remaining non-ranked IDs
            for (const id of ids) {
              if (!tieredIdSet.has(id) && !rankedIds.includes(id)) rankedIds.push(id);
            }
        ...
        logger.error('SEARCH', 'Tiered search failed for decisions, falling back to metadata search', {}, tieredError as Error);
      }
    }
    ...
    if (this.queryRouter) {
      try {
        ...
        if (allIds.size > 0) {
          ...
          const tieredResults = await this.queryTiered('what changed', Math.min(idsArray.length, 100));
          const tieredIdSet = new Set(tieredResults.obsIds);
        ...
        logger.error('SEARCH', 'Tiered search failed for changes, falling back to metadata search', {}, tieredError as Error);
      }
    }
  ...
  async howItWorks(args: any): Promise<any> {
    ...
    if (this.queryRouter) {
      ...
      if (metadataResults.length > 0) {
        const ids = metadataResults.map(obs => obs.id);
        const tieredResults = await this.queryTiered('how it works architecture', Math.min(ids.length, 100));
        const tieredIdSet = new Set(tieredResults.obsIds);
  ...
  async searchObservations(args: any): Promise<any> {
    ...
    // Vector-first search via tiered storage (pgvector)
    if (this.queryRouter) {
      logger.debug('SEARCH', 'Using tiered semantic search (pgvector + SQLite)', {});

      // Step 1: Tiered semantic search (top 100)
      const tieredResults = await this.queryTiered(query, 100);
      logger.debug('SEARCH', 'Tiered search returned semantic matches', { matchCount: tieredResults.obsIds.length });

      if (tieredResults.obsIds.length > 0) {
        // Step 2: Hydrate from SQLite in temporal order
        const limit = options.limit || 20;
        results = this.sessionStore.getObservationsByIds(tieredResults.obsIds, { orderBy: 'date_desc', limit });
  ...
  async searchSessions(args: any): Promise<any> {
    ...
    // Vector-first search via tiered storage (pgvector)
    if (this.queryRouter) {
      logger.debug('SEARCH', 'Using tiered semantic search for sessions', {});

      // Step 1: Tiered semantic search (top 100)
      const tieredResults = await this.queryTiered(query, 100);
      logger.debug('SEARCH', 'Tiered search returned semantic matches for sessions', { matchCount: tieredResults.sessionIds.length });

      if (tieredResults.sessionIds.length > 0) {
        // Step 2: Hydrate from SQLite in temporal order
        const limit = options.limit || 20;
        results = this.sessionStore.getSessionSummariesByIds(tieredResults.sessionIds, { orderBy: 'date_desc', limit });
  ...
  async searchUserPrompts(args: any): Promise<any> {
    ...
    // Vector-first search via tiered storage (pgvector)
    if (this.queryRouter) {
      logger.debug('SEARCH', 'Using tiered semantic search for user prompts', {});

      // Step 1: Tiered semantic search (top 100)
      const tieredResults = await this.queryTiered(query, 100);
      logger.debug('SEARCH', 'Tiered search returned semantic matches for prompts', { matchCount: tieredResults.obsIds.length });

      if (tieredResults.obsIds.length > 0) {
        // Step 2: Hydrate from SQLite in temporal order
        const limit = options.limit || 20;
        results = this.sessionStore.getUserPromptsByIds(tieredResults.obsIds, { orderBy: 'date_desc', limit });
  ...
  async findByConcept(args: any): Promise<any> {
    ...
    if (this.queryRouter) {
      ...
      if (metadataResults.length > 0) {
        // Step 2: Tiered semantic ranking (rank by relevance to concept)
        const ids = metadataResults.map(obs => obs.id);
        const tieredResults = await this.queryTiered(concept, Math.min(ids.length, 100));
        const tieredIdSet = new Set(tieredResults.obsIds);
        ...
        logger.debug('SEARCH', 'Tiered search ranked results by semantic relevance', { count: rankedIds.length });
    ...
    // Fall back to SQLite-only if tiered search unavailable or failed
    if (results.length === 0) {
  ...
  async findByFile(args: any): Promise<any> {
    ...
    if (this.queryRouter) {
      ...
      if (metadataResults.observations.length > 0) {
        // Step 2: Tiered semantic ranking (rank by relevance to file path)
        const ids = metadataResults.observations.map(obs => obs.id);
        const tieredResults = await this.queryTiered(filePath, Math.min(ids.length, 100));
        const tieredIdSet = new Set(tieredResults.obsIds);
        ...
        logger.debug('SEARCH', 'Tiered search ranked observations by semantic relevance', { count: rankedIds.length });
  ...
  async findByType(args: any): Promise<any> {
    ...
    if (this.queryRouter) {
      ...
      if (metadataResults.length > 0) {
        // Step 2: Tiered semantic ranking (rank by relevance to type)
        const ids = metadataResults.map(obs => obs.id);
        const tieredResults = await this.queryTiered(typeStr, Math.min(ids.length, 100));
        const tieredIdSet = new Set(tieredResults.obsIds);
  ...
  async getTimelineByQuery(args: any): Promise<any> {
    ...
    // Use tiered search if available
    if (this.queryRouter) {
      logger.debug('SEARCH', 'Using tiered semantic search for timeline query', {});
      const tieredResults = await this.queryTiered(query, 100);
      logger.debug('SEARCH', 'Tiered search returned semantic matches for timeline', { matchCount: tieredResults.obsIds.length });
      ...
      if (tieredResults.obsIds.length > 0) {
        results = this.sessionStore.getObservationsByIds(tieredResults.obsIds, { orderBy: 'date_desc', limit: mode === 'auto' ? 1 : limit });
        logger.debug('SEARCH', 'Hydrated observations from SQLite', { count: results.length });
      }
    }
