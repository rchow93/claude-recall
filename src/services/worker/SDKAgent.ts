 ...
 * - Sync to database and tiered storage
 */
...
// Maximum time to wait for a single SDK message before considering the session stalled
const SDK_MESSAGE_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes

export class SDKAgent {
  private dbManager: DatabaseManager;
  private sessionManager: SessionManager;
  private cachedClaudePath: string | null = null; // Cache to avoid repeated execSync calls
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    // Process SDK messages with timeout protection
    // If no message is received within SDK_MESSAGE_TIMEOUT_MS, abort the session
    let lastMessageTime = Date.now();
    const timeoutChecker = setInterval(() => {
      const elapsed = Date.now() - lastMessageTime;
      if (elapsed > SDK_MESSAGE_TIMEOUT_MS) {
        logger.error('SDK', 'Message timeout - aborting stalled session', {
          sessionId: session.sessionDbId,
          timeoutMs: SDK_MESSAGE_TIMEOUT_MS,
          lastMessageAgoMs: elapsed
        });
    ...
    for await (const message of queryResult) {
      ...
      // Always clear the timeout checker to prevent memory leaks
  ...
  /**
   * Find Claude executable (cached after first lookup to avoid repeated execSync calls)
   * The cache is per-instance, so if settings change the worker should be restarted.
   */
  private findClaudeExecutable(): string {
    // Return cached path if available (avoids blocking execSync on every session)
    ...
    if (settings.CLAUDE_CODE_PATH) {
      ...
      this.cachedClaudePath = settings.CLAUDE_CODE_PATH;
      return this.cachedClaudePath;
    }

    // 2. Try auto-detection (blocking, but only happens once due to cache)
    try {
      ...
      if (claudePath) {
        this.cachedClaudePath = claudePath;
        logger.debug('SDK', 'Claude executable found and cached', { path: claudePath });
        return claudePath;
      }
    } catch (error) {
    ...
    throw new Error('Claude executable not found. Please either:\n1. Add "claude" to your system PATH, or\n2. Set CLAUDE_CODE_PATH in ~/.claude-recall/settings.json');
  }
  ...
  private getModelId(): string {
    const settingsPath = path.join(homedir(), '.claude-recall', 'settings.json');
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
    return settings.CLAUDE_RECALL_MODEL;
  }
}
