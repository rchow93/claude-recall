...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      if (!apiKey) {
        throw new Error('OpenRouter API key not configured. Set CLAUDE_RECALL_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
      }
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES = parseInt(settings.CLAUDE_RECALL_OPENROUTER_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
    const MAX_ESTIMATED_TOKENS = parseInt(settings.CLAUDE_RECALL_OPENROUTER_MAX_TOKENS) || DEFAULT_MAX_ESTIMATED_TOKENS;
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/askqai/claude-recall',
        'X-Title': appName || 'claude-recall',
  ...
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    ...
    const apiKey = settings.CLAUDE_RECALL_OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY || '';

    // Model: from settings or default
    const model = settings.CLAUDE_RECALL_OPENROUTER_MODEL || 'xiaomi/mimo-v2-flash:free';

    // Optional analytics headers
    const siteUrl = settings.CLAUDE_RECALL_OPENROUTER_SITE_URL || '';
    const appName = settings.CLAUDE_RECALL_OPENROUTER_APP_NAME || 'claude-recall';
...
export function isOpenRouterAvailable(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.CLAUDE_RECALL_OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY);
}
...
export function isOpenRouterSelected(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.CLAUDE_RECALL_PROVIDER === 'openrouter';
}
