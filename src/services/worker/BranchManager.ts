 ...
 * The installed plugin at ~/.claude/plugins/marketplaces/askqai/ is a git repo.
 */
...
const INSTALLED_PLUGIN_PATH = join(homedir(), '.claude', 'plugins', 'marketplaces', 'askqai');
...
export async function switchBranch(targetBranch: string): Promise<SwitchResult> {
  ...
  try {
    ...
    // 1. Discard local changes (safe - user data is at ~/.claude-recall/)
