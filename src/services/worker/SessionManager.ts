...
// Maximum messages in queue per session to prevent unbounded growth
// If exceeded, new observations are dropped with a warning
const MAX_QUEUE_DEPTH_PER_SESSION = 1000;
...
export class SessionManager {
   ...
   */
  queueObservation(sessionDbId: number, data: ObservationData): boolean {
    ...
    const currentDepth = this.getPendingStore().getPendingCount(sessionDbId);
    if (currentDepth >= MAX_QUEUE_DEPTH_PER_SESSION) {
      logger.warn('QUEUE', `Queue depth limit reached, dropping observation`, {
        sessionId: sessionDbId,
        currentDepth,
        maxDepth: MAX_QUEUE_DEPTH_PER_SESSION,
        tool: data.tool_name
      });
      return false; // Indicate observation was dropped
    }
    ...
    return true; // Observation was queued successfully
  }
  ...
  /**
   * Get list of active sessions (for storage details)
   */
