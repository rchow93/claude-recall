...
const ROOT = join(homedir(), '.claude', 'plugins', 'marketplaces', 'askqai');
...
try {
  ...
  // Cache resolved bun path for bun-runner.sh
  const resolvedBunPath = getBunPath();
  if (resolvedBunPath && resolvedBunPath !== 'bun') {
    try {
      writeFileSync(join(ROOT, '.bun-path'), resolvedBunPath);
    } catch {
      // Non-critical - bun-runner.sh will fall back to PATH lookup
    }
  }
